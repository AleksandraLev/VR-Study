...
public class CardboardReticlePointer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, _RETICLE_MAX_DISTANCE))
        {
            ...
            {
                // New GameObject.
                if (_gazedAtObject != null && IsInteractive(_gazedAtObject))
                {
                    _gazedAtObject.GetComponent<ObjectController>()?.OnPointerExit();
                ...
                {
                    _gazedAtObject.GetComponent<ObjectController>()?.OnPointerEnter();
        ...
        if (Gamepad.current.rightTrigger.isPressed || Gamepad.current.leftTrigger.isPressed)
        {
            OnPointerClick();
        }
    ...
    public void OnPointerClick()
    {
        if (IsInteractive(_gazedAtObject))
        {
            _gazedAtObject.GetComponent<ObjectController>()?.OnPointerClick();
